...
def genDeathRates(csvFile):
	...
	for x in list_of_countries:
			...
			confirmed.append(df.loc[df['Country/Region'] == x, 'Confirmed'].sum())
			deaths.append(df.loc[df['Country/Region'] == x, 'Deaths'].sum())
			recovered.append(df.loc[df['Country/Region'] == x, 'Recovered'].sum())

			deathRates.append(df.loc[df['Country/Region'] == x, 'Deaths'].sum() / df.loc[df['Country/Region'] == x, 'Confirmed'].sum())
			recoveryRates.append(df.loc[df['Country/Region'] == x, 'Recovered'].sum() / df.loc[df['Country/Region'] == x, 'Confirmed'].sum())
